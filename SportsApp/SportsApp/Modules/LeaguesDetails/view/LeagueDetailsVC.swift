...
class LeagueDetailsVC: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
       ...
       favouriteButton()
        ...
        //        fetchJSON(from: "https://apiv2.allsportsapi.com/tennis/?met=Fixtures&APIkey=REDACTED_ALLSPORTS_API_KEY&from=\(getDatesForUrl().0)&to=\(getDatesForUrl().1)", as: tennisResponse.self) { result in
        //            print(result)
        //        }
    }
    ...
    func fetchUpcomingEvents(){
        ...
        let baseUrl = URL(string: "https://apiv2.allsportsapi.com/\(String(describing: sport!))/?met=Fixtures&APIkey=REDACTED_ALLSPORTS_API_KEY&from=\(currentDay)&to=\(nextDay)")
    ...
    func fetchlatestResults(){
        ...
        let baseUrl = URL(string: "https://apiv2.allsportsapi.com/\(sport!)/?met=Fixtures&APIkey=REDACTED_ALLSPORTS_API_KEY&from=\(dayBefore)&to=\(currentDay)")
        AF.request(baseUrl!).responseDecodable(of: footballResponse.self) { response in
            switch response.result {
            case .success(let eventResponse):
                for item in 0...10 {
