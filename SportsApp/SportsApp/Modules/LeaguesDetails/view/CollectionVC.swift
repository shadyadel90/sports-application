...
class CollectionVC: UICollectionViewController {
    ...
    func fetchUpcomingEvents(){
        ...
        let baseUrl = URL(string: "https://apiv2.allsportsapi.com/football/?met=Fixtures&APIkey=REDACTED_ALLSPORTS_API_KEY&from=\(currentDay)&to=\(nextDay)")
    ...
    func fetchlatestResults(){
        ...
        let baseUrl = URL(string: "https://apiv2.allsportsapi.com/football/?met=Fixtures&APIkey=REDACTED_ALLSPORTS_API_KEY&from=\(dayBefore)&to=\(currentDay)")
        AF.request(baseUrl!).responseDecodable(of: EventsResponse.self) { response in
            switch response.result {
            case .success(let eventResponse):
                for item in 0...10 {
